...
def test_plumbing_component_setup():
    ...
    assert pc1.states == {'open': {(1, 2, 'A1'): 4500.0, (2, 1, 'A2'): 4500.0}, 'closed': {(1, 2, 'A1'): 0.0045, (2, 1, 'A2'): 0.0045}}
...
def test_plumbing_engine_setup():
    ...
    plumb = ops.PlumbingEngine([pc1, pc2], component_mapping, pressures, default_states)

    assert plumb.plumbing_graph.edges(data=True, keys=True) == [(1, 2, 'A1', {'FC': 0.0045}), (2, 1, 'A2', {'FC': 0.0045}), (2, 3, 'B1', {'FC': 4500.0}), (3, 2, 'B2', {'FC': 4500.0})]
    assert plumb.time_resolution == 5e-05
    assert plumb.plumbing_graph.nodes(data=True) == [(1, {'pressure': 0}), (2, {'pressure': 0}), (3, {'pressure': 100})]
